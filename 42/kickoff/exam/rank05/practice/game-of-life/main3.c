...
void free_board_partial(t_board *board, int grid_alloc)
{
  if (!board)
    return;

  for (int i = 0; i < grid_alloc; i++)
    free(board->grid[i]);
  free(board->grid);
  free(board);
}
...
void process_cmd(t_board *board, t_pen *pen, char cmd)
{
  int new_x = pen->x;
  int new_y = pen->y;
  ...
  else if (cmd == 'x')
  {
    ...
    // draw if pen down (with pen points)
    if (pen->pen_down)
  ...
  if (new_x >= 0 && new_x < board->width && new_y >= 0 && new_y < board->height)
  {
    ...
    // draw if pen down (with updated pen points)
    if (pen->pen_down)
      board->grid[pen->y][pen->x] = 'O';
  }
}
