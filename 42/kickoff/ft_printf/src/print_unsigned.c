...
/*   Updated: 2025/05/15 00:46:24 by kaahmed          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	print_unsigned(int fd, unsigned int n, t_flags flags)
{
	...
	if (!flags.left_align)
	{
		vars.ret = ft_putpad(fd, flags.width, vars.len + vars.zeros,
				flags.zero_pad);
		if (!safe_count(vars.ret, &vars.count))
			return (-1);
	}
	vars.ret = ft_putbuf(fd, buf, vars.len, vars.zeros);
	if (!safe_count(vars.ret, &vars.count))
		return (-1);
	if (flags.left_align)
	{
		vars.ret = ft_putpad(fd, flags.width, vars.len + vars.zeros, 0);
		if (!safe_count(vars.ret, &vars.count))
			return (-1);
	}
	return (vars.count);
